...
function getDivisiFrom(data) {
  for (const key in data) {
    if (Object.hasOwnProperty.call(data, key)) { const
      object = data[key], index = object['Index'], namaLengkap = object['Nama Lengkap'],
      nomorWA = object['Nomor WhatsApp'], alasan = object['Alasan anda untuk bergabung di TECO'],
      jurusan = object['Kompetensi Keahlian'], divisi = object['Divisi'];
      switch (divisi) {
        case "Pemrograman Java": java.push(jurusan); siswa_java.push([index, namaLengkap, nomorWA, divisi, jurusan, alasan]); break;
        case "Website": website.push(jurusan); siswa_website.push([index, namaLengkap, nomorWA, divisi, jurusan, alasan]); break;
        case "Desain Grafis": grafis.push(jurusan); siswa_grafis.push([index, namaLengkap, nomorWA, divisi, jurusan, alasan]); break;
        case "Office": office.push(jurusan); siswa_office.push([index, namaLengkap, nomorWA, divisi, jurusan, alasan]); break;
        case "Jaringan": jaringan.push(jurusan); siswa_jaringan.push([index, namaLengkap, nomorWA, divisi, jurusan, alasan]); break;
        case "Animasi": animasi.push(jurusan); siswa_animasi.push([index, namaLengkap, nomorWA, divisi, jurusan, alasan]); break;
...
function viewSiswaFrom(data) {
  for (let i = 0; i < data.length; i++) {
    const siswa = data[i]; key = siswa[3];
    switch (key) {
      case "Pemrograman Java": siswa[3] = 'Java'; break;
      case "Desain Grafis": siswa[3] = 'Grafis'; break;
    }
    document.getElementById("tabel-body").innerHTML +=
      `
      <tr>
        <td class="text-center">${i+1}</td>
        <td>${siswa[1].toUpperCase()}</td>
        <td>${siswa[2]}</td>
        <td>${siswa[3]}</td>
        <td>${siswa[4]}</td>
        <td style="width: 25%">${siswa[5]}</td>
      </tr>`;
  }
}
...
function viewDivisiFrom(query) {
  if (query === null) { label_all = divisi;
    ...
    siswa_all = [...siswa_java, ...siswa_website, ...siswa_grafis, ...siswa_office, ...siswa_jaringan, ...siswa_animasi]
    ...
    pushToChartJS(label_all, data_all, warna_all, border_all); viewSiswaFrom(siswa_all);
  } else { siswa_divisi = eval("siswa_"+query);
